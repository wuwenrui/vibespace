...
function generateEntrypoint(config) {
  ...
  if (isCnb) {
    ...
    lines.push('');
    lines.push('    # 1. 复制目标目录/文件到 staging');
    ...
    lines.push('            fi');
    lines.push('        elif [ -f "$path" ]; then');
    lines.push('            # 文件：复制到对应父目录');
    lines.push('            local rel_path="${path#/}"');
    lines.push('            local parent_dir="$(dirname "$rel_path")"');
    lines.push('            local target_parent="$staging_dir/$parent_dir"');
    lines.push('            mkdir -p "$target_parent"');
    lines.push('            if ! cp -a "$path" "$target_parent/"; then');
    lines.push('                echo "[OSS] 复制失败: $path"');
    lines.push('                copy_failed=1');
    lines.push('            else');
    lines.push('                echo "[OSS] 已复制: $path"');
    lines.push('            fi');
    lines.push('        else');
    lines.push('            echo "[OSS] 跳过（不存在）: $path"');
    lines.push('        fi');
    lines.push('    done');
    lines.push('');
    ...
    lines.push('    for path in "${PATHS[@]}"; do');
    lines.push('        if [ -d "$path" ]; then');
    lines.push('            # 目录：备份整个目录');
    lines.push('            local rel_path="${path#/}"');
    lines.push('            mkdir -p "$backup_dir/$rel_path"');
    lines.push('            cp -a "$path/." "$backup_dir/$rel_path/" 2>/dev/null || true');
    lines.push('        elif [ -f "$path" ]; then');
    lines.push('            # 文件：备份到对应父目录');
    lines.push('            local rel_path="${path#/}"');
    lines.push('            local parent_dir="$(dirname "$rel_path")"');
    lines.push('            mkdir -p "$backup_dir/$parent_dir"');
    lines.push('            cp -a "$path" "$backup_dir/$parent_dir/" 2>/dev/null || true');
    lines.push('        fi');
    lines.push('    done');
    lines.push('');
    lines.push('    # 4. 清空目标目录/删除目标文件');
    lines.push('    echo "[OSS] 清空目标..."');
    lines.push('    for path in "${PATHS[@]}"; do');
    lines.push('        if [ -d "$path" ]; then');
    lines.push('            # 目录：清空内容');
    ...
    lines.push('        elif [ -f "$path" ]; then');
    lines.push('            # 文件：直接删除');
    lines.push('            rm -f "$path" 2>/dev/null || true');
    lines.push('        fi');
    lines.push('    done');
    lines.push('');
    lines.push('    # 5. 解包恢复');
    ...
    lines.push('                # 目录：恢复整个目录');
    lines.push('                mkdir -p "$path"');
    lines.push('                cp -a "$backup_dir/$rel_path/." "$path/" 2>/dev/null || true');
    lines.push('            elif [ -f "$backup_dir/$rel_path" ]; then');
    lines.push('                # 文件：恢复单个文件');
    lines.push('                local parent_dir="$(dirname "$path")"');
    lines.push('                mkdir -p "$parent_dir"');
    lines.push('                cp -a "$backup_dir/$rel_path" "$path" 2>/dev/null || true');
    lines.push('            fi');
    lines.push('        done');
    ...
    lines.push('        elif [ -f "$staging_dir/$rel_path" ]; then');
    lines.push('            # 文件：恢复单个文件');
    lines.push('            local parent_dir="$(dirname "$path")"');
    lines.push('            mkdir -p "$parent_dir"');
    lines.push('            cp -a "$staging_dir/$rel_path" "$path" 2>/dev/null || true');
